...
Snapshot version 7 canonically includes living and casualty state, resource accounting, due transitions, treatments, and work counters; derived membership and medic indexes are reconstructed during strict restore. The benchmark checksum covers projected living fields, life/death details, activity, materialization time, health, and carried inventory. Earlier snapshot versions are deliberately rejected. Time has a closed terminal instant at `u64::MAX`; no transition is scheduled beyond it. The manual benchmark defaults to 2,410,000 records and 100,000 dense commands. This increment does not implement weapons, tactical combat, vehicles, routes, higher formations, communications, AI, graphics, or setting assets.

## M1.2 casualty care

Typed authoritative commands can now inflict distinct persistent wounds and request or directly start timed care from real same-faction, same-cell `Role::Medic` entities. Hemostasis and shock care consume conserved carried medical inventory at start and only change physiology on completion; interruptions never refund supplies. Snapshot v7 includes casualty, wound, treatment, allocator, and medical-ledger state. See [ADR 0003](docs/adr/0003-casualty-care.md) and the [M1.2 acceptance map](docs/m1-2-acceptance-map.md). This is casualty care, not weapon or combat simulation.
