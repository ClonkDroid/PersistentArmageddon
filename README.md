...
Cold soldiers are indexed by their next discrete consequence and lazily projected for pure queries, so unrelated scheduled timestamps do not materialize or visit them. Hot cells step only their indexed living members with the same one-second transition. At a shared timestamp, automatic events are globally ordered by entity ID before commands execute in schedule-ID order. Failed commands and advances publish no partial state or events. Hot cells retain fixed-step telemetry, but the real living-member work is reported separately. See [ADR 0002](docs/adr/0002-living-state-transition-engine.md).
