...
| --- | --- |
| 1. Independent reference/cold matrix | `private_invariants::cold_analytical_path_matches_hand_authored_duration_matrix` (20 literal cold-path rows spanning all activities, four food/water availability combinations, threshold-adjacent states, targets 0/1/boundary-1/boundary/boundary+1/multiple boundaries/huge terminal time, and complete state/event/index/accounting assertions); `private_invariants::independent_transition_oracle_threshold_matrix` (15 supplemental pure-transition rows); `cluster_01_one_second_oracle_has_independent_expected_values`; `oracle_matrix_duration_zero_is_an_exact_noop`; `cluster_11_huge_sparse_advance_has_exact_boundary_work` |
| 2. Hot/cold boundaries | `private_invariants::cold_analytical_path_matches_hand_authored_duration_matrix` independently fixes the cold analytical results; `private_invariants::cold_and_hot_paths_each_match_hand_authored_boundary_fixtures` separately checks the hot path at ration, forced-idle, deterioration, and death boundaries; `cluster_02_hot_and_cold_match_independent_boundaries` remains supplemental cross-path parity coverage |
...
| 9. Snapshot validation/query purity | `private_invariants::v6_byte_corruption_matrix_rejects_every_living_class` (39 named byte/encoded-field mutations with exact `SimError` categories, including stale materialization for a living hot member); `cluster_09_hot_snapshot_and_canonical_due_are_strict`; `query_permutations_are_snapshot_and_digest_pure`; existing M0 corruption tests in `sim-core/tests/invariants.rs` |
