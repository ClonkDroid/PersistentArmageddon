    ...
    pub fn from_snapshot(b: &[u8]) -> Result<Self, SimError> {
        ...
        for _ in 0..r.u32()? {
            ...
            if status == TreatmentStatus::Active {
                if completes_at <= clock || !soldiers.valid(medic) || !soldiers.valid(patient) {
                    return Err(SimError::Snapshot("active treatment"));
                }
                if active_by_entity.contains_key(&medic) || active_by_entity.contains_key(&patient)
                {
                    return Err(SimError::Snapshot("active treatment conflict"));
                }
                active_by_entity.insert(medic, id);
                active_by_entity.insert(patient, id);
            }
        }
        if r.p != b.len() {
...
mod private_invariants {
    ...
    #[test]
    fn gate_c1_2a_r6_active_treatment_endpoint_conflicts() {
        fn read_u32(bytes: &[u8], at: usize) -> u32 {
            u32::from_le_bytes(bytes[at..at + 4].try_into().unwrap())
        }
        fn read_u64(bytes: &[u8], at: usize) -> u64 {
        ...
        fn scalar_bytes(at: usize, before: u64, after: u64) -> Vec<usize> {
            let before = before.to_le_bytes();
            let after = after.to_le_bytes();
            (0..8)
                .filter_map(|i| (before[i] != after[i]).then_some(at + i))
        ...
        for (index, patient) in endpoints.iter().copied().enumerate() {
            let wound = WoundSpec {
                trauma: 0,
                bleeding_per_second: u16::from(index == 1),
                shock: 400,
            };
            let outcome = world.apply(Command::InflictWound { patient, wound });
            let id = match outcome.events.as_slice() {
                [TimedEvent {
                    at: 1,
                    event: Event::WoundInflicted { id, patient: p, .. },
                }] if *p == patient => *id,
        ...
        let started0 = world.apply(Command::StartTreatment {
            medic: endpoints[0],
            patient: endpoints[1],
            wound: Some(wound_ids[1]),
            kind: TreatmentKind::Hemostatic,
        });
        let treatment0 = match started0.events.as_slice() {
            [TimedEvent {
                at: 1,
                event: Event::TreatmentStarted { id, .. },
            }] => *id,
            events => panic!("unexpected first treatment events: {events:?}"),
        };
        let started1 = world.apply(Command::StartTreatment {
            medic: endpoints[2],
            patient: endpoints[3],
            wound: None,
            kind: TreatmentKind::Shock,
        });
        let treatment1 = match started1.events.as_slice() {
            [TimedEvent {
                at: 1,
                event: Event::TreatmentStarted { id, .. },
            }] => *id,
            events => panic!("unexpected second treatment events: {events:?}"),
        };
        assert_eq!((treatment0, treatment1), (TreatmentId(0), TreatmentId(1)));

        let expected_casualties = endpoints
            .iter()
            .copied()
            .map(|id| {
                (
        ...
        let expected_wounds = endpoints
            ...
            .map(|(i, patient)| {
                (
                    ...
                    Wound {
                        ...
                        spec: WoundSpec {
                            trauma: 0,
                            bleeding_per_second: u16::from(i == 1),
                            shock: 400,
                        },
                        controlled: false,
                        healed: false,
            ...
            .collect::<BTreeMap<_, _>>();
        let expected_treatments = BTreeMap::from([
            (
                treatment0,
                Treatment {
                    id: treatment0,
                    ...
                    kind: TreatmentKind::Hemostatic,
                    started_at: 1,
                    completes_at: 1 + HEMOSTATIC_DURATION,
                    consumed: HEMOSTATIC_COST,
                    status: TreatmentStatus::Active,
                },
            ),
            (
                ...
                Treatment {
                    ...
                    kind: TreatmentKind::Shock,
                    started_at: 1,
                    completes_at: 1 + SHOCK_TREATMENT_DURATION,
                    consumed: SHOCK_TREATMENT_COST,
                    status: TreatmentStatus::Active,
                },
            ),
        ]);
        let expected_active = endpoints
            .iter()
            .copied()
            .enumerate()
            .map(|(i, id)| (id, if i < 2 { treatment0 } else { treatment1 }))
            .collect::<BTreeMap<_, _>>();
        let expected_due = BTreeMap::from([
            (1 + HEMOSTATIC_DURATION, BTreeSet::from([treatment0])),
            (1 + SHOCK_TREATMENT_DURATION, BTreeSet::from([treatment1])),
        ]);
        let expected_reverse_due = BTreeMap::from([
            (treatment0, 1 + HEMOSTATIC_DURATION),
            (treatment1, 1 + SHOCK_TREATMENT_DURATION),
        ]);
        let expected_history = endpoints
            .iter()
            .copied()
            .enumerate()
            .map(|(i, id)| {
                (
                    id,
                    BTreeSet::from([if i < 2 { treatment0 } else { treatment1 }]),
                )
            })
            .collect::<BTreeMap<_, _>>();
        let expected_membership = endpoints
            .iter()
            .copied()
            .enumerate()
            .map(|(i, id)| (id, BTreeSet::from([WoundId(i as u64)])))
            .collect::<BTreeMap<_, _>>();
        let expected_totals = ResourceTotals {
            ammunition: 0,
            stockpile_supplies: 0,
            ...
            lost_water: 0,
            sourced_medical: 32,
            carried_medical: 32 - u128::from(HEMOSTATIC_COST + SHOCK_TREATMENT_COST),
            consumed_medical: u128::from(HEMOSTATIC_COST + SHOCK_TREATMENT_COST),
            lost_medical: 0,
        };
        let assert_control = |control: &World| {
            assert_eq!(control.clock, 1);
            for (i, id) in endpoints.iter().copied().enumerate() {
                assert!(control.soldiers.valid(id));
                let spec = control.soldiers.data[id.index()];
                let living = control.soldiers.living[id.index()];
                assert_eq!(spec.role, Role::Medic);
                assert_eq!((spec.faction, spec.position.cell), (0, 0));
                assert_eq!(living.life, LifeState::Alive);
                assert_eq!(living.activity, Activity::Idle);
                assert_eq!(living.health, 1000);
                assert_eq!(living.materialized_at, 1);
                assert!(!control.is_incapacitated(id));
                assert_eq!(spec.inventory.medical, [7, 8, 6, 8][i]);
            }
            assert_eq!(control.casualty, expected_casualties);
            assert_eq!(control.wounds, expected_wounds);
            assert_eq!(control.treatments, expected_treatments);
            assert_eq!(control.next_wound_id, 4);
            assert_eq!(control.next_treatment_id, 2);
            assert_eq!(control.active_by_entity, expected_active);
            assert_eq!(control.treatment_due, expected_due);
            assert_eq!(control.due_by_treatment, expected_reverse_due);
            assert_eq!(control.treatment_ids_by_entity, expected_history);
            assert_eq!(control.wound_ids_by_patient, expected_membership);
            assert_eq!(
                control.bleeding_rate_by_patient,
                BTreeMap::from([(endpoints[1], 1)])
            );
            ...
            assert_eq!(control.sourced_medical, 32);
            assert_eq!(
                control.consumed_medical,
                u128::from(HEMOSTATIC_COST + SHOCK_TREATMENT_COST)
            );
            assert_eq!(control.lost_medical, 0);
            assert_eq!(
                expected_totals.sourced_medical,
                expected_totals.carried_medical
                    + expected_totals.consumed_medical
        ...
        let digest = world.state_digest();
        let layout = V7MedicalLayout::parse(&bytes);
        assert_eq!(read_u32(&bytes, layout.treatment_count), 2);
        assert_eq!(layout.treatments.len(), 2);
        assert_eq!(layout.treatments[0].range.start, layout.treatment_count + 4);
        assert_eq!(
            layout.treatments[0].range.end,
            layout.treatments[1].range.start
        );
        assert_eq!(layout.treatments[1].range.end, layout.end);
        assert_eq!(layout.end, bytes.len());
        let restored = World::from_snapshot(&bytes).unwrap();
        ...
        let assert_record =
            |image: &[u8], parsed: &V7MedicalLayout, collision: usize, replacement: EntityId| {
                assert_eq!(read_u32(image, parsed.treatment_count), 2);
                assert_eq!(parsed.treatments[0].range, layout.treatments[0].range);
                assert_eq!(parsed.treatments[1].range, layout.treatments[1].range);
                assert_eq!(parsed.end, image.len());
                for (i, expected) in expected_treatments.values().enumerate() {
                    let encoded = &parsed.treatments[i];
                    assert_eq!(read_u64(image, encoded.id), expected.id.0);
                    assert_eq!(
                        read_u64(image, encoded.medic),
                        if i == 1 && collision == encoded.medic {
                            replacement.raw()
                        } else {
                            expected.medic.raw()
                        }
                    ...
                    assert_eq!(
                        read_u64(image, encoded.patient),
                        if i == 1 && collision == encoded.patient {
                            replacement.raw()
                        } else {
                            expected.patient.raw()
                        }
                    );
                    assert_eq!(
                        image[encoded.wound_option],
                        u8::from(expected.wound.is_some())
                    );
                    assert_eq!(
                        encoded.wound.map(|at| read_u64(image, at)),
                        expected.wound.map(|id| id.0)
                    );
                    assert_eq!(
                        image[encoded.kind],
                        if expected.kind == TreatmentKind::Hemostatic {
                            0
                        } else {
                            1
                        }
                    );
                    assert_eq!(read_u64(image, encoded.started_at), expected.started_at);
                    assert_eq!(read_u64(image, encoded.completes_at), expected.completes_at);
                    assert_eq!(read_u32(image, encoded.consumed), expected.consumed);
                    assert_eq!(image[encoded.status], 0);
                }
            };
        for (field, replacement) in [
            (layout.treatments[1].medic, endpoints[0]),
            (layout.treatments[1].medic, endpoints[1]),
            (layout.treatments[1].patient, endpoints[1]),
            (layout.treatments[1].patient, endpoints[0]),
        ] {
            assert!(world.soldiers.valid(replacement));
            assert_eq!(world.soldiers.data[replacement.index()].role, Role::Medic);
            assert_eq!(
                world.soldiers.living[replacement.index()].life,
                LifeState::Alive
            );
            assert_eq!(
                world.soldiers.living[replacement.index()].activity,
                Activity::Idle
            );
            assert!(!world.is_incapacitated(replacement));
            assert_eq!(
                world.casualty[&replacement],
                expected_casualties[&replacement]
            );
            let untouched = if field == layout.treatments[1].medic {
                endpoints[3]
            } else {
                endpoints[2]
            };
            assert_ne!(replacement, untouched);
            let original = read_u64(&bytes, field);
            let mut changed = bytes.clone();
            put_u64(&mut changed, field, replacement.raw());
            let changed_layout = V7MedicalLayout::parse(&changed);
            assert_record(&changed, &changed_layout, field, replacement);
            assert_eq!(
                changed_bytes(&bytes, &changed),
                scalar_bytes(field, original, replacement.raw())
            );
            assert_eq!(world.next_wound_id, 4);
            assert_eq!(world.next_treatment_id, 2);
            assert_eq!(world.resource_totals(), expected_totals);
            assert_eq!(world.wounds, expected_wounds);
            assert_eq!(world.casualty, expected_casualties);
            assert_eq!(
                World::from_snapshot(&changed).err(),
    ...
    fn gate_c1_mixed_world_continuation_is_literal_and_deterministic() {
        ...
        let expected_initial_casualty = BTreeMap::from([
            (
                p3,
                CasualtyState {
                    blood: 4_980,
        ...
        let expected_initial_history = BTreeMap::from([
            (m0, BTreeSet::from([TreatmentId(2)])),
            (m1, BTreeSet::from([TreatmentId(0)])),
            (m2, BTreeSet::from([TreatmentId(1)])),
            (p3, BTreeSet::from([TreatmentId(0)])),
            (p4, BTreeSet::from([TreatmentId(1)])),
            (p5, BTreeSet::from([TreatmentId(2)])),
        ]);
        let expected_initial_ownership = BTreeMap::from([
            (p3, BTreeSet::from([WoundId(0)])),
            (p4, BTreeSet::from([WoundId(1)])),
            (p5, BTreeSet::from([WoundId(2)])),
        ]);
        let expected_initial_living_due = BTreeMap::from([
            (15, BTreeSet::from([p3])),
            (50, BTreeSet::from([m0, m1, m2, p4, p5])),
        ]);
        let expected_initial_due_by_entity =
            HashMap::from([(m0, 50), (m1, 50), (m2, 50), (p3, 15), (p4, 50), (p5, 50)]);
        let expected_initial_available = BTreeMap::from([
            ((0, 0, 1), BTreeSet::from([m1, m2])),
            ((0, 0, 2), BTreeSet::from([m1, m2])),
        ]);
        let expected_initial_availability_reverse = BTreeMap::from([
        ...
        fn run_suffix(mut world: World, label: &str) -> (World, Vec<TimedEvent>) {
            ...
            assert_eq!(world.active_by_entity.get(&m0), Some(&TreatmentId(2)));
            assert!(!world.availability_by_medic.contains_key(&m0));
            assert_eq!(
                world.availability_by_medic[&m1],
                BTreeSet::from([(0, 0, 1), (0, 0, 2)])
            );
            assert_eq!(
                world.availability_by_medic[&EntityId::from_parts(2, 0)],
                BTreeSet::from([(0, 0, 1), (0, 0, 2)])
            );
            assert_eq!(
                world.available_medics[&(0, 0, 1)],
                BTreeSet::from([m1, EntityId::from_parts(2, 0)])
            );
            assert_eq!(world.soldiers.data[m0.index()].inventory.medical, 5);
            assert_eq!(world.soldiers.data[m1.index()].inventory.medical, 5);
            assert_eq!(world.soldiers.data[2].inventory.medical, 5);
            ...
            assert_eq!(world.lost_medical, 0);
            assert!(!world.casualty.contains_key(&p5));
            assert!(!world.wound_ids_by_patient.contains_key(&p5));
            assert!(!world.active_by_entity.contains_key(&p5));
            assert!(!world.treatment_ids_by_entity.contains_key(&p5));
            assert!(!world.due_by_entity.contains_key(&p5));
            assert!(!world.availability_by_medic.contains_key(&p5));
            assert!(world
                .cell_members
                .values()
                .all(|members| !members.contains(&p5)));
            assert!(world.wounds.values().all(|wound| wound.patient != p5));
            assert!(world
                .treatments
                .values()
    ...
    fn gate_c1_all_six_death_causes_preserve_medical_history() {
        ...
        struct ExpectedDeath {
            clock: u64,
            medic_living: LivingState,
            patient_living: LivingState,
            medic_inventory: Inventory,
            patient_inventory: Inventory,
        ...
        fn literal_spec(role: Role, inventory: Inventory) -> SoldierSpec {
            SoldierSpec {
                faction: 0,
                position: Position {
                    x_mm: 0,
        ...
        fn history_world(
            ...
            let patient_inventory = Inventory {
                food,
                water,
                medical: 0,
            };
            ...
            assert_eq!(wound_outcome.clock, 0);
            assert_eq!(wound_outcome.error, None);
            assert_eq!(wound_outcome.blocked, None);
            assert_eq!(
                wound_outcome.events,
            ...
            let treatment = TreatmentId(0);
            assert_eq!(treatment_outcome.clock, 0);
            assert_eq!(treatment_outcome.error, None);
            assert_eq!(treatment_outcome.blocked, None);
            assert_eq!(
                treatment_outcome.events,
        ...
        fn assert_fixture(
            ...
            medic: EntityId,
            patient: EntityId,
            expected: &ExpectedDeath,
        ) {
            assert!(world.soldiers.valid(medic), "{label}: medic generation");
            assert!(world.soldiers.valid(patient), "{label}: patient generation");
            assert_eq!(world.clock, expected.clock, "{label}: clock");
            assert_eq!(
                world.soldiers.living[medic.index()],
                expected.medic_living,
                "{label}: medic living"
            );
            assert_eq!(
                world.soldiers.living[patient.index()],
                expected.patient_living,
                "{label}: patient living"
            );
            assert_eq!(
                world.soldiers.data[medic.index()].health,
                expected.medic_living.health,
                "{label}: medic health mirror"
            );
            assert_eq!(
                world.soldiers.data[patient.index()].health,
                expected.patient_living.health,
                "{label}: patient health mirror"
            );
            assert_eq!(
                world.soldiers.data[medic.index()],
                SoldierSpec {
                    faction: 0,
                    position: Position {
                        x_mm: 0,
                    ...
                    role: Role::Medic,
                    rank: 0,
                    health: expected.medic_living.health,
                    ammunition: 0,
                    inventory: expected.medic_inventory,
                },
                "{label}: medic authority"
            );
            assert_eq!(
                world.soldiers.data[patient.index()],
                SoldierSpec {
                    faction: 0,
                    position: Position {
                        x_mm: 0,
                    ...
                    role: Role::Rifle,
                    rank: 0,
                    health: expected.patient_living.health,
                    ammunition: 0,
                    inventory: expected.patient_inventory,
                },
                "{label}: patient authority"
            );
            assert_eq!(
                world.casualty,
                BTreeMap::from([(patient, expected.casualty)]),
                "{label}: casualty map"
            );
            assert_eq!(world.wounds, expected.wounds, "{label}: wounds");
            assert_eq!(
                world.treatments,
                BTreeMap::from([(expected.treatment.id, expected.treatment)]),
                "{label}: treatments"
            );
            assert_eq!(
            ...
            assert_eq!(
                world.wound_ids_by_patient,
                BTreeMap::from([(patient, expected.wounds.keys().copied().collect())]),
                "{label}: wound ownership"
            );
            assert_eq!(
                world.bleeding_rate_by_patient, expected.bleeding,
                "{label}: bleeding aggregate"
            );
            assert_eq!(
                world.treatment_ids_by_entity,
                BTreeMap::from([
                    (medic, BTreeSet::from([TreatmentId(0)])),
                    (patient, BTreeSet::from([TreatmentId(0)]))
                ]),
                "{label}: treatment history"
            );
            assert!(
                world.active_by_entity.is_empty(),
                "{label}: active endpoints"
            );
            assert!(world.treatment_due.is_empty(), "{label}: treatment due");
            assert!(
                world.due_by_treatment.is_empty(),
                "{label}: treatment reverse due"
            );
            if expected.available {
                let keys = BTreeSet::from([(0, 0, HEMOSTATIC_COST), (0, 0, SHOCK_TREATMENT_COST)]);
                assert_eq!(
                    world.availability_by_medic,
                    BTreeMap::from([(medic, keys)]),
                    "{label}: availability reverse"
                );
                assert_eq!(
                    world.available_medics,
                    BTreeMap::from([
                        ((0, 0, HEMOSTATIC_COST), BTreeSet::from([medic])),
                        ((0, 0, SHOCK_TREATMENT_COST), BTreeSet::from([medic]))
                    ]),
                    "{label}: availability forward"
                );
                assert_eq!(
                    world.due_by_entity,
                    HashMap::from([(medic, 400)]),
                    "{label}: living reverse due"
                );
                assert_eq!(
                    world.living_due,
                    BTreeMap::from([(400, BTreeSet::from([medic]))]),
                    "{label}: living due"
                );
            } else {
                assert!(
                    world.availability_by_medic.is_empty(),
                    "{label}: no availability reverse"
                );
                assert!(
                    world.available_medics.is_empty(),
                    "{label}: no availability forward"
                );
                assert!(
                    world.due_by_entity.is_empty(),
                    "{label}: no living reverse due"
                );
                assert!(world.living_due.is_empty(), "{label}: no living due");
            }
            assert_eq!(
                world.resource_totals(),
                expected.totals,
                "{label}: resource totals"
            );
            assert_eq!(
                expected.totals.sourced_medical,
                expected
                    .totals
                    .carried_medical
                    .checked_add(expected.totals.consumed_medical)
                    .and_then(|v| v.checked_add(expected.totals.lost_medical))
                    .unwrap(),
                "{label}: medical conservation"
            );
            assert_eq!(
                expected.totals.sourced_food,
                expected
                    .totals
                    .carried_food
                    .checked_add(expected.totals.consumed_food)
                    .and_then(|v| v.checked_add(expected.totals.lost_food))
                    .unwrap(),
                "{label}: food conservation"
            );
            assert_eq!(
                expected.totals.sourced_water,
                expected
                    .totals
                    .carried_water
                    .checked_add(expected.totals.consumed_water)
                    .and_then(|v| v.checked_add(expected.totals.lost_water))
                    .unwrap(),
        ...
        fn assert_death_round_trip(
            ...
            assert_fixture("original", world, medic, patient, &expected);
            let original_bytes = world.snapshot();
            let original_digest = world.state_digest();
            let mut first_restore = World::from_snapshot(&original_bytes).unwrap();
            assert_eq!(
                first_restore.snapshot(),
                original_bytes,
                "first restore canonical bytes"
            );
            assert_eq!(
                first_restore.state_digest(),
                original_digest,
                "first restore digest"
            );
            assert_fixture("first restore", &first_restore, medic, patient, &expected);

            let first_bytes = first_restore.snapshot();
            let mut second_restore = World::from_snapshot(&first_bytes).unwrap();
            assert_eq!(
                second_restore.snapshot(),
                original_bytes,
                "second restore canonical bytes"
            );
            assert_eq!(
                second_restore.state_digest(),
                original_digest,
                "second restore digest"
            );
            assert_fixture("second restore", &second_restore, medic, patient, &expected);

            for (name, restored) in [
                ("first restore", &mut first_restore),
                ("second restore", &mut second_restore),
            ] {
                let target = expected.clock + 20;
                let probe = restored.apply(Command::AdvanceTo { target });
                assert_eq!(probe.clock, target, "{name}: probe clock");
                assert_eq!(probe.error, None, "{name}: probe error");
                assert_eq!(probe.blocked, None, "{name}: probe blocked");
                assert_eq!(
                    probe.events,
                    vec![TimedEvent {
                        at: target,
        ...
        for (
            ...
            assert_eq!(death.clock, 2_000);
            assert_eq!(death.error, None);
            assert_eq!(death.blocked, None);
            let relevant: Vec<_> = death
                .events
                .iter()
                .copied()
                .filter(|e| {
                ...
                vec![
                    ...
                    TimedEvent {
                        ...
                        event: Event::SoldierDied {
                            id: patient,
                            cause,
                            health_before: if cause == DeathCause::Starvation {
                                4
                            } else {
                                10
                            }
        ...
        for cause in [DeathCause::ImmediateTrauma, DeathCause::TraumaticShock] {
            ...
            let fatal_wound = WoundId(1);
            assert_eq!(fatal.clock, 0);
            assert_eq!(fatal.error, None);
            assert_eq!(fatal.blocked, None);
            assert_eq!(
                fatal.events,
                vec![
                    TimedEvent {
            ...
            let probe = world.apply(Command::AdvanceTo { target: 20 });
            assert_eq!(probe.clock, 20);
            ...
            let casualty = if cause == DeathCause::TraumaticShock {
                CasualtyState {
                    blood: BLOOD_MAX,
                    shock: 1000,
                    shock_remainder: 0,
            ...
            assert_death_round_trip(
                ...
                ExpectedDeath {
                    ...
                    patient_living: LivingState {
                        ...
                        health: 0,
                        activity: Activity::Idle,
                        life: LifeState::Dead { at: 0, cause },
                        materialized_at: 0,
                    },
                    medic_inventory: Inventory {
                        food: 0,
        ...
        let second = world.apply(Command::InflictWound {
            patient,
            wound: second_spec,
        });
        let second_wound = WoundId(1);
        assert_eq!(second.clock, 0);
        assert_eq!(second.error, None);
        assert_eq!(second.blocked, None);
        assert_eq!(
            second.events,
            vec![TimedEvent {
                at: 0,
        ...
        assert_eq!(interrupted.error, None);
        assert_eq!(interrupted.blocked, None);
        assert_eq!(
            interrupted.events,
            vec![TimedEvent {
                at: 0,
